  ...
  applyPlugins(_ref) {
    ...
    if (plugins) {
      plugins.forEach(plugin => {
        if (result) {
          try {
            const pluginResult = plugin.execute(result); // Each destination is independent from each other, so we don't roll over changes caused internally in each one of their processing

            if (type !== _types.PluginType.destination) {
              result = pluginResult;
            }
          } catch (error) {
            console.warn(`Destination ${plugin.key} failed to execute: ${JSON.stringify(error)}`);
          }
        }
      });
    }

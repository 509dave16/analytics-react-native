...
var _default = payload => {
  if (payload.event === 'Install Attributed') {
    ...
    if ((_payload$properties = payload.properties) !== null && _payload$properties !== void 0 && _payload$properties.campaign) {
      ...
      _reactNativeAppboySdk.default.setAttributionData(network, campaign, adGroup, creative);
    }
  }
    ...
    if (payload.properties) {
      ...
      if (appBoyProperties.products) {
        ...
        products.forEach(product => {
          ...
          _reactNativeAppboySdk.default.logPurchase(productId, String(productRevenue), currency, productQuantity, productProperties);
        });
      } else {
        _reactNativeAppboySdk.default.logPurchase(payload.event, String(revenue), currency, 1, appBoyProperties);
      }
    } else {
      _reactNativeAppboySdk.default.logPurchase(payload.event, String(revenue), currency, 1);
    }
  } else {
    _reactNativeAppboySdk.default.logCustomEvent(payload.event, payload.properties);
  }
